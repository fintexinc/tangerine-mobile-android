                ...
                Canvas(
                    ...
                    data.forEachIndexed { index, line ->
                        ...
                        if(index == 0) {
                            ...
                            // Direction vector for 45° angle (normalize so step size is uniform)
                            val dir = Offset(1f, -1f)
                            val len = hypot(dir.x, dir.y)
                            val ux = dir.x / len
                            val uy = dir.y / len

                            // Perpendicular vector for width expansion
                            val px = -uy
                            val py = ux

                            val start = Offset(0f, h)
                            val end = Offset(w, 0f) // top-right
                            ...
                            for (i in 0..steps) {
                                ...
                                // Interpolate along diagonal line directly
                                val x = start.x + (end.x - start.x) * t
                                val y = start.y + (end.y - start.y) * t

                                val halfWidth = minWidth / 2 + (maxWidth - minWidth) / 2 * t

                                leftSide.add(Offset(x + px * halfWidth, y + py * halfWidth))
                                rightSide.add(Offset(x - px * halfWidth, y - py * halfWidth))
                            }

                            // Build ribbon path
                            val path = Path().apply {
                                leftSide.forEachIndexed { index, p ->
                                    if (index == 0) moveTo(p.x, p.y) else lineTo(p.x, p.y)
                                }
                                rightSide.reversed().forEach { p ->
                                    lineTo(p.x, p.y)
                                }
                                close()
                            }
                            ...
                            drawPath(
                                path = path,
                                color = Color(0xFFF3FDD8) // soft green
                            )
                        }
                        drawPath(
                            path = segmentedPath,
